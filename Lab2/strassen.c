...
int main(int argc, char *argv[])
{
	...
	pthread_create(&tid[0], NULL, f, &thread_id[0]);

	// calculul matricii M2
	pthread_create(&tid[1], NULL, f1, &thread_id[1]);

	// calculul matricii M3
	pthread_create(&tid[2], NULL, f2, &thread_id[2]);

	// calculul matricii M4
	pthread_create(&tid[3], NULL, f3 ,&thread_id[3]);

	// calculul matricii M5
	pthread_create(&tid[4], NULL, f4, &thread_id[4]);

	// calculul matricii M6
	pthread_create(&tid[5], NULL, f5, &thread_id[5]);

	// calculul matricii M7
	pthread_create(&tid[6], NULL, f6, &thread_id[6]);
